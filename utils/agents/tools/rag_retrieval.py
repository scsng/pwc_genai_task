...
class RAGRetrieval:
    """RAG retrieval tool class that wraps vector database search."""
    
    def __init__(self, vector_db: QdrantDB):
        """Initialize RAG retrieval with a vector database.
        
        Args:
            vector_db: QdrantDB instance for document search.
        """
        self.vector_db = vector_db
        self.tool = self._create_tool()
    
    def _create_tool(self):
        """Create the LangChain tool with bound vector_db."""
        vector_db = self.vector_db  # Capture in closure
